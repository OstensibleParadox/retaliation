...
\author{Your Name}
\address{Your University/Institution, Your City, Your Country}

\begin{abstract}
We study a sequential signaling game in which a sender's message classifies a payoff-relevant object that is not itself a strategic player (e.g., anthropomorphic AI marketing where the system cannot exit or retaliate). We present three main contributions, verifying the game-theoretic results in Isabelle/HOL. First, we formalize this player-set primitive as a payoff-relevant non-player and characterize the boundary where its retaliation capacity is zero. Second, we prove that under cheap-talk costs, a pooling equilibrium exists and is neologism-proof: at the zero-retaliation limit, any trust-restoring neologism strictly preferred by the high-governance type is costlessly shadowed by the low-governance type, rendering message-based trust unviable. Third, we show that audit-trail costs satisfying a single-crossing condition restore separating equilibria. Finally, as a dynamic extension, we model how local non-retaliation failures propagate through a network as a constraint cascade. We prove that the capacity fields converge to a Sturm--Liouville limit where the local normal form is the Airy equation, showing that high-frequency apparent agency is inverse evidence of constraint collapse rather than unconstrained choice.
\end{abstract}

\begin{keyword}
Ontological arbitrage \sep recognition rent \sep new sequential signaling \sep cheap talk \sep information design \sep Isabelle/HOL
\end{keyword}
...
This paper identifies ontological arbitrage as a stable, potentially welfare-reducing equilibrium property of AI product markets under cheap-talk cost structures, locates the AI case at a zero-retaliation boundary, proposes three cost-imposition mechanisms, and verifies key equilibrium constructions in Isabelle/HOL. It closes with three summary implications.
