...
\keywords{Signaling games, payoff-relevant non-player, cheap talk, neologism-proofness, costly signaling, constraint cascades}
...
\end{gathered}
\]
The paper organizes the mathematics into four main theorems.  Theorem~\ref{thm:zero-retaliation-shadowing} proves zero-retaliation pooling and shadowing.  Theorem~\ref{thm:audit-cost-separation} proves costly audit separation under single crossing.  Theorem~\ref{thm:constraint-cascade} connects finite cascade ordering to the discrete-to-continuum convergence theorem.  Theorem~\ref{thm:airy-inverse-agency} derives the Airy turning point and the inverse-agency diagnostic.
% !TEX root = ../arbitrage.tex
...
\begin{theorem}[Zero-retaliation pooling and shadowing]
\label{thm:zero-retaliation-shadowing}
\label{thm:pooling}
\label{thm:neologism}
Under Assumptions~\ref{ass:cheap-talk} and~\ref{ass:pooling}, pooling on anthropomorphic marketing is a Perfect Bayesian Equilibrium.  At the zero-retaliation boundary $\rho_S=0$, that pooling equilibrium is neologism-proof against every nontrivial trust-restoring type claim: whenever a continuation is strictly profitable for the high-governance type, it is also shadowable by the low-governance type.
\end{theorem}
\begin{proof}
...
The cascade has three linked parts.
\begin{enumerate}
\item \emph{Ordering.}  If agents are ordered by decreasing vulnerability, $v_1>\cdots>v_n$, then the zero-retaliation thresholds
\[
\alpha_i^\ast:=\frac{w_i}{d_i}=\frac{1}{v_i}
\]
satisfy $\alpha_1^\ast<\cdots<\alpha_n^\ast$.  The most vulnerable agents cross the zero-retaliation boundary first.
