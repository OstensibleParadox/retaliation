...
% !TEX root = ../arbitrage.tex
\section{From Finite Games to Unpriced-Arbitrage Cascades}
\label{sec:cascade}

The dynamic extension is not needed for the pooling or separation results.  Its role is evidentiary: it describes what observers may see when profitable cheap talk is not disciplined over time.  If unpriced zero-retaliation failures persist across a dependency network, one actor's loss of effective retaliation capacity can lower the bargaining power of adjacent actors.  This section records the finite cascade and the continuum limit used in the Airy analysis.  The continuum field is not imposed as an analogy; it is obtained as the weak limit of discrete retaliation-capacity fields.
...
\begin{theorem}[Inverse-agency diagnostic after cascade collapse]
