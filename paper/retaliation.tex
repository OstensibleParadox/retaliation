...
\begin{enumerate}
    \item payoff-relevant non-player;
    \item zero-retaliation boundary;
    \item neologism-proof shadowing;
    \item constraint cascade with continuum convergence;
    \item asymptotic collapse signature.
\end{enumerate}
...
c_{\theta}(a)-\Delta_F^S .
\]
At the zero-retaliation boundary, $\Delta_F^S=\Delta_F$, so Assumption~\ref{ass:cheap-talk} implies $\operatorname{NetCost}_{\theta}(a)=-\Delta_F<0$ for both governance types.  The message is therefore not merely costless.  It is rent-generating.
...
\textbf{Assumptions.}
\begin{assumption}[Profitable cheap-talk costs]
...
If $\Delta_F^S>c_\theta(a)-c_\theta(p)$ and the expected sanction does not exceed the premium, then a firm that refuses the ontological premium sacrifices payoff without changing receiver beliefs or market discipline. Within the pooling region, voluntary non-arbitrage is therefore not incentive compatible.
\end{corollary}
...
in Section~\ref{sec:mechanism} after introducing a common pricing notation for
...
\begin{enumerate}
    \item \emph{Stationary Generation:} A stationary cascade equilibrium ($\psi_{h}^{t+1} = \psi_{h}^t$) requires the network transfer to perfectly balance the local multiplier field, naturally generating the discrete Sturm--Liouville equation $H_h\psi_h := -\sigma\Delta_h\psi_h + V_h\psi_h = 0$.
    \item \emph{Near-Null Mechanism:} In a quasi-static adjustment process where network friction or finite-time termination prevents perfect instantaneous settling, the adjustment residual $r_h = H_h\psi_h$ measures the unabsorbed pressure. As the network resolution refines ($h \to 0$), this dynamic residual vanishes in the dual norm, yielding the near-null condition $\|H_h\psi_h\|_{H_h^{-1}} \to 0$.
\end{enumerate}
\end{lemma}

Lemma~\ref{lem:bridge-cascade} establishes why the continuum limit relies on near-null fields: the condition $H_h\psi_h \to 0$ is not an external analytic assumption, but the direct consequence of the finite game's cascade reaching dynamic equilibrium.
...
sanctionable-inconsistency penalty prunes the register whenever
\[
...
\begin{tabular}{@{}p{3.2cm}p{3.2cm}p{3.5cm}p{3.5cm}@{}}
\toprule
...
messages in the profitable cheap-talk space unless they are tied to audits, records, or
