...
Sturm--Liouville problems.  The bridge argument in the main text identifies the
near-null condition as the required mesh-consistency assumption for a
quasi-static cascade refinement: finite-time adjustment supplies a residual, and
the continuum theorem applies to refinement sequences for which that residual
vanishes in the discrete dual norm.  The point is not to impose an Airy profile
by analogy, but first to prove that these near-null discrete fields have a
continuum weak limit for the Sturm--Liouville operator.
