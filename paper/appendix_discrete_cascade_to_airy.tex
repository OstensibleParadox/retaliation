...
Sturm--Liouville problems.  The point is not to impose an Airy profile by
analogy, but first to prove that near-null discrete fields have a continuum
...
\[
  ...
  \tag{A.3}
\]
Equivalently, $-\sigma\Psi''+V\Psi=0$ in $H^{-1}(I)$.  Near any simple
turning point of $V$, this weak limit has the Airy normal form described in
Appendix~\ref{app:airy-normal-form}.
\end{theorem}
...
\[
  a_h^\gamma(u,u)
  \ge
  \sigma h\sum_{i=1}^{n+1}|(D_hu)_i|^2
  +
  \|u\|_{L_h^2}^2 ,
  \tag{A.5}
\]
after decreasing the right-hand constant if necessary.  This is the Garding
shift: the positive shift supplies coercivity without claiming that the
sign-changing operator $H_h$ is coercive on its own.
...
let $\eta_h\in X_h$ be its nodal restriction, $\eta_{h,i}=\eta(x_i)$.  The
residual assumption gives
\[
  a_h(\psi_h,\eta_h)
  =
